  ...
  try {
    ...
    if (!data.capabilities) {
      missionSelect.innerHTML =
        '<option value="">(No capabilities provided)</option>';
      return;
    }
    ...
    if (caps.perception_modules) {
      for (const [moduleName, info] of Object.entries(
        caps.perception_modules
      )) {
    ...
    console.error(err);
    missionSelect.innerHTML =
      '<option value="">Error loading missions</option>';
  }
}
...
const droneMarker = L.marker([37.422, -122.084], { icon: DroneIcon }).addTo(
  map
);
  ...
  if (e.layerType === "polyline") {
    ...
    try {
      ...
      for (let i = 1; i < pts.length; i++)
        meters += pts[i - 1].distanceTo(pts[i]);
...
async function runMission() {
  ...
  if (config.geometry_type === "point") {
    if (!setpointMarker)
      return alert("Click on the map to set a target point.");
...
function updateHUD(data) {
  ...
  if (["READY", "MANUAL_FLIGHT", "MISSION", "COMPLETE"].includes(data.status))
    stateEl.classList.add("status-green");
  else if (
    [
      "INITIALIZING",
      "PREFLIGHT_CHECK",
    ...
    stateEl.classList.add("status-yellow");
  else stateEl.classList.add("status-red");

  if (
    ["RTL_88", "MANUAL_FLIGHT", "MISSION", "TAKING_OFF", "LANDING"].includes(
      data.status
    )
  )
    droneInFlight = true;
...
async function renderMissionDetails(config) {
  ...
  const required = Array.isArray(config.requires_activation)
    ? config.requires_activation
    : [];
...
function openSettings() {
  ...
  document.getElementById("pilotName").value =
    localStorage.getItem("pilotName") || "";
  document.getElementById("pilotLicense").value =
    localStorage.getItem("pilotLicense") || "";
}
